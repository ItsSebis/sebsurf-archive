...
if (isset($_POST["send"])) {
    ...
    if ($role == "null" || empty($subject) || empty($text)) {
    ...
    foreach (roleMemberArray($con, $role) as $user) {
        sendNotification($con, $user, $_SESSION["username"], $subject." [".roleData($con, $role)["name"]."]", $text);
    }
    header("location: notifications.php?usr=".roleData($con, $role)["name"]."&error=send");
    exit();
...
<div class="main">
    ...
    <?php
        ...
        if (isset($_GET["error"])) {
            if ($_GET["error"] == "send") {
                echo "<p style='color: lime; border: solid green; max-width: 500px; text-align: center; margin: 10px auto; border-radius: 7px;'>Es wurde erfolgreich ein Nachricht an alle der Rolle ".$_GET["usr"]." gesendet!</p>";
            }
        }
    ?>
</div>
