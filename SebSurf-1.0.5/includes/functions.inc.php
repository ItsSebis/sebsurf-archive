    ...
    while ($row = $rs->fetch_assoc()) {
      echo '<div class="main" style="width: 40%;">';
        ...
        echo("
          ...
          <p style='color: grey;'>von ".userData($con, $row["publisher"])['fullname']." (".roleData($con, userData($con, $row['publisher'])['role'])["name"].")</p><br>
        ");
      echo '</div>';
    }
  }
...
function setupTableTickets($con) {
...
function setupTableTicketInputs($con) {
  $sql = "CREATE TABLE `ticketinputs` (
    `id` INT(11) NOT NULL,
    `by` VARCHAR(64) NOT NULL,
...
function editUserAccountNameTickets($con, $old, $new) {
  $qry = "UPDATE tickets SET by=? WHERE by=?";
  $stmt = mysqli_stmt_init($con);
  if (mysqli_stmt_prepare($stmt, $qry)) {
    mysqli_stmt_bind_param($stmt, "ss", $new, $old);
    mysqli_stmt_execute($stmt);
...
function editUserAccountNameTicketInputs($con, $old, $new) {
  $qry = "UPDATE ticketinputs SET by=? WHERE by=?";
  $stmt = mysqli_stmt_init($con);
  if (mysqli_stmt_prepare($stmt, $qry)) {
    mysqli_stmt_bind_param($stmt, "ss", $new, $old);
    mysqli_stmt_execute($stmt);
