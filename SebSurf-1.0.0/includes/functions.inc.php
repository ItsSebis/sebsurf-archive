...
function deleteNews($con) {
  $qry = "DELETE FROM news WHERE id=?;";
    ...
    mysqli_stmt_bind_param($stmt, "s", currentNewsData($con)["id"]);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
}
...
function createUser($con, $name, $admin) {
  ...
  $sql = "INSERT INTO users (account, usrpw) VALUES (?, ?);";
  $stmt = mysqli_stmt_init($con);
  if (!mysqli_stmt_prepare($stmt, $sql)) {
    header("location: ../?error=1");
    exit();
  ...
  header("location: ../admin.php?error=usercreated&name=".$name);
  exit();
...
function userList($con) {
  ...
  if ($rs->num_rows > 0) {
    echo '<select name="user" id="users" style="background-color: #303030; outline: none; color: white; border: solid #333333; border-radius: 24px; width: 200px; height: 70px; padding: 14px 10px; transition: 0.2s; font-size: larger;">';
...
function rolesList($con) {
  ...
  if ($rs->num_rows > 0) {
    echo '<select name="role" id="roles" style="background-color: #303030; outline: none; color: white; border: solid #333333; border-radius: 24px; width: 200px; height: 70px; padding: 14px 10px; transition: 0.2s; font-size: larger;">';
...
function notifyTable($con, $usr) {
  ...
  if ($rs->num_rows > 0) {
    ...
    while ($row = $rs->fetch_assoc()) {
      ...
      echo "<tr>";
      echo "  <td style='border: 2px solid black;'>".$row['sender']."</td>";
    ...
    echo "<p style='color: lime;'>Du hast keine Mitteilungen!</p>";
  }
  ...
  if ($rs->num_rows > 0) {
    while ($row = $rs->fetch_assoc()) {
      ...
      <div class='main'>
        ...
        <h3>Sender: ".$row['sender']."</h3><br>
        <p>Content:</p>
        <p id='text' style='width: 60%; border: 2px solid #404040; margin: 0 auto; text-align: left; padding: 5px; border-radius: 10px;'>".$row['text']."</p><br>
        <a href='./notifications.php' style='border: solid white; padding: 2px; border-radius: 5px;'>← Zurück<a>
      ";
    }
  }
    ...
    while ($row = $rs->fetch_assoc()) {
      if ($row["gid"] != 0) {
        ...
        <tr>
          ...
          <td style='border: 2px solid black;'>".$row['name']."</td>
          <td style='border: 2px solid black;'>".$row['createdby']."</td>
...
function teamDatas($con, $team) {
  if ($team == "null") {
    ...
    $sql = "SELECT * FROM data WHERE team=? ORDER BY `lessons` DESC;";
  }
  $stmt = mysqli_stmt_init($con);
  if (!mysqli_stmt_prepare($stmt, $sql)) {
...
function versions($con) {
  ...
  if ($rs->num_rows > 0) {
    echo '
        ...
        <thead>
          <tr>
            <th style="padding-left: 10px; padding-right: 10px;">Version</th>
            <th style="padding-left: 10px; padding-right: 10px;">Herausgeber</th>
            <th style="padding-left: 10px; padding-right: 10px;">Datum (GMT)</th>
            <th style="padding-left: 10px; padding-right: 10px;">Neues</th>
          </tr>
        </thead>
        <tbody><br>
    ';
    while ($row = $rs->fetch_assoc()) {
      ...
      echo "
        <tr>
          <td style='border: 2px solid black;'>".$row['version']."</td>
          <td style='border: 2px solid black;'>".$row['updater']."</td>
          <td style='border: 2px solid black;'>".$row['update']."</td>
          <td style='border: 2px solid black;'>".$row['actions']."</td>
        </tr>
      ";
    }
    echo '
    ...
    echo "<p style='color: red;'>Es sind keine Versionen vorhanden!</p>";
  }
...
function users($con) {
  $sql = "SELECT * FROM users ORDER BY `role` ASC, `disabled` ASC, `account` ASC;";
  $stmt = mysqli_stmt_init($con);
  if (!mysqli_stmt_prepare($stmt, $sql)) {
    header("location: index.php?error=1");
    exit();
  ...
  if ($rs->num_rows > 0) {
    while ($row = $rs->fetch_assoc()) {
      ...
      <tr>
        <td style='border: 2px solid black;'>".$row['account']."</td>
        <td style='border: 2px solid black;'>".$row['nick']."</td>
    ...
    echo "<p style='color: red;'>There are no users!</p>";
  }
...
function delData($con, $id) {
  ...
  header("location: ../profile.php?error=deldata&name=".$data["name"]);
  exit();
}
...
function createRootUser($con) {
  $sql = "INSERT INTO users (account, usrpw, `role`, disabled) VALUES (?, ?, ?, ?);";
  $stmt = mysqli_stmt_init($con);
  if (!mysqli_stmt_prepare($stmt, $sql)) {
  ...
  $arg3 = "0";
  $arg4 = "1";
  $hashedPwd = password_hash("root", PASSWORD_DEFAULT);

  mysqli_stmt_bind_param($stmt, "ssss", $arg1, $hashedPwd, $arg3, $arg4);
  mysqli_stmt_execute($stmt);
  mysqli_stmt_close($stmt);
}
